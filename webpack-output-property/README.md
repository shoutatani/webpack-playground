...
- [output.library.type オプション](https://webpack.js.org/configuration/output/#outputlibrarytype) は、それぞれ以下の通り使えることがわかった。
  ...
  - output.library.type オプションが、モジュールシステムを用いたモジュールの受け渡しの場合
    ...
    - ["type: "jsonp"](https://webpack.js.org/configuration/output/#type-jsonp) の場合
      - バンドルされたエントリーポイントで、 `<ライブラリ名>(エクスポートするものの集合);` と、紐付けれ、自動で呼び出しがかかることが見て取れた。
      - したがって、予め `<ライブラリ名>` という名前でバンドルしたファイルを読み込む空間に関数を定義しておき、バンドルされたファイルを読み込めば、エクスポートしたものが使えるようになることがわかった。
